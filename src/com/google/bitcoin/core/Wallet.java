...
public class Wallet implements Serializable {
    ...
    /**
     * Build an unsigned {@link Transaction} to be returned to the client for
     * signing. This allows transaction building and signing to be broken into
     * a two stage process which remain independent of one another.
     * 
     * NOTE: This method also supports multiple outputs and automatically
     * calculates and includes the required network fees for the size of
     * the transaction.
     * 
     * @param address
     * @param nanocoins
     ...
     * @return
     */
    public Transaction createSendUnsigned(List<Address> addresses,
    		List<BigInteger> nanocoins, Address changeAddress,
    		BigInteger fee) throws InsufficientFundsException {
    	
    	// Calculate the total required coins for all outputs,
    	// including the network fee
    	BigInteger totalSend = BigInteger.ZERO;
    	totalSend = totalSend.add(fee);
    	for (BigInteger nanocoin : nanocoins) {
    		totalSend = totalSend.add(nanocoin);
    	} // for
    	
    	List<Integer> keyidList = new ArrayList<Integer>();
        ...
        for (Transaction tx : unspent.values()) {
            for (TransactionOutput output : tx.outputs) {
                ...
                try {
                	keyid =
                		findKeyIndexFromPubHash(output.getScriptPubKey().getPubKeyHash());
                } catch (Exception ex) {}
                if (keyid == -1) {
                	log.error("Negative key index from keyring.");
                	return null;
                }
                keyidList.add(keyid);
                valueGathered = valueGathered.add(output.getValue());
                log.info("gathered: " + output.getValue());
            }
            if (valueGathered.compareTo(totalSend) >= 0) break;
        }
        log.info("total: " + valueGathered);
        
        // Can we afford this?
        if (valueGathered.compareTo(totalSend) < 0) {
            log.info("Insufficient value in wallet for send, missing " +
                    bitcoinValueToFriendlyString(totalSend.subtract(valueGathered)));
            throw new InsufficientFundsException("The funds are too damn low.");
        }
        assert gathered.size() > 0;
        Transaction sendTx = new Transaction(params);
        ...
        for (int x = 0; x < addresses.size(); x++) {
        	...
        	BigInteger coins = nanocoins.get(x);
        	sendTx.addOutput(new TransactionOutput(params, sendTx, coins, address));
        }
        
        // Calculate and add change output
        BigInteger change = valueGathered.subtract(totalSend);
        if (change.compareTo(BigInteger.ZERO) > 0) {
            ...
            // Get the index of the key for the change address
            int keyid = findKeyIndexFromPubHash(changeAddress.getHash160());
            if (keyid == -1) return null;
            else keyidList.add(keyid);
        ...
        // Convert out collected key indices into an int array
        int[] keyids = new int[keyidList.size()];
        for (int x = 0; x < keyidList.size(); x++) {
        	keyids[x] = keyidList.get(x);
        } // for
        sendTx.keyIndices = keyids;
        
        return sendTx;
    } // createSendUnsigned

    /**
     * Sign a transaction which contains unsigned inputs.
     * 
     * @param unsignedTx	Transaction, likely from a remote server, which has
     * 						not been signed yet.
     * @return
     */
    public Transaction signTransaction(Transaction unsignedTx) {
    	try {
    		unsignedTx.signInputs(Transaction.SigHash.ALL, this);
    	} catch (Exception ex) {
    		log.error("We couldn't sign the transaction. :(");
    ...
    /**
     * Send a transaction directly to our connected peers.
     * 
     * @param peerGroup
     * @param tx
     * @return
     * @throws IOException
     */
    public synchronized Transaction sendTx(PeerGroup peerGroup, Transaction tx) throws IOException {
        if (!peerGroup.broadcastTransaction(tx)) {
            throw new IOException("Failed to broadcast tx to all connected peers");
        }
        confirmSend(tx);
    ...
    public synchronized int findKeyIndexFromPubHash(byte[] pubkeyHash) {
        for (int x = 0; x < keychain.size(); x++) {
        	ECKey key = keychain.get(x);
            if (Arrays.equals(key.getPubKeyHash(), pubkeyHash)) return x;
        }
        return -1;
    }
